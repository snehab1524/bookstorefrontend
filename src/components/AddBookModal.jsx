...
const AddBookModal = ({ isOpen, onClose }) => {
  const { addBook, isAdmin } = useBooks();
  ...
  useEffect(() => {
    if (isOpen) {
      // Reset form
      setFormData({ title: '', author: '', price: '', category: '', imageUrl: '' });
      setError('');
    }
  }, [isOpen]);
      ...
      <div className="bg-zinc-800 rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            className="w-full py-3 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-600 disabled:cursor-not-allowed rounded-lg font-semibold text-lg transition-all duration-300 flex items-center justify-center gap-2"
          >
            <FaPlus />
